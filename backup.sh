...
logLast "BACKUP_VOLUME: ${BACKUP_VOLUME}"
logLast "BACKUP_FOLDER: ${BACKUP_FOLDER}"

if [ -d "${BACKUP_VOLUME}/${BACKUP_FOLDER}" ]; then
    # Do not save full backup log to logfile but to backup-last.log
    restic backup ${BACKUP_VOLUME}/${BACKUP_FOLDER} ${RESTIC_JOB_ARGS} --tag=${RESTIC_TAG?"Missing environment variable RESTIC_TAG"} >> ${lastLogfile} 2>&1
    rc=$?
    ...
    echo "data folder missing. No Backup creation!"
fi
